...
const wchar_t* D3D12RaytracingAmbientOcclusion::c_rayGenShaderNames[] = 
{
	...
	L"MyRayGenShader_GBuffer", L"MyRayGenShader_AO", L"MyRayGenShader_AO_sortedRays", L"MyRayGenShaderQuarterRes_AO", L"MyRayGenShader_Visibility", L"MyRayGenShader_ShadowMap"
};
...
namespace SceneArgs
{
    ...
    IntVar GrassGeometry_NumberStrawsX(L"Scene2/Grass/Patch/Num Straws X", 100, 1, MAX_GRASS_STRAWS_1D, 10);
    IntVar GrassGeometry_NumberStrawsZ(L"Scene2/Grass/Patch/Num Straws Z", 100, 1, MAX_GRASS_STRAWS_1D, 10);
    NumVar GrassGeometry_PatchLengthX(L"Scene2/Grass/Patch/Length X", 14, 1, 100, 1);
    NumVar GrassGeometry_PatchLengthY(L"Scene2/Grass/Patch/Length Y", 1, 0.1f, 10, 0.1f);
    NumVar GrassGeometry_PatchLengthZ(L"Scene2/Grass/Patch/Length Z", 14, 1, 100, 1);
    NumVar GrassGeometry_StrawHeight(L"Scene2/Grass/Patch/Straw Height", 0.4f, 0.1f, 10, 0.05f);
    ...
    NumVar GrassGeometry_WindStrength(L"Scene2/Grass/Wind strength", 0.4f, 0, 1, 0.02f);
    NumVar GrassGeometry_WindFrequency(L"Scene2/Grass/Wind frequency", 0.04f, 0, 1, 0.005f);
    NumVar GrassGeometry_RandomPositionJitterStrength(L"Scene2/Grass/Randomness/Position jitter strength", 1.0f, 0, 2, 0.05f);
    ...
    */
    ...
    const WCHAR* FloatingPointFormatsRGB[RGBResourceFormats::Count] = {
    L"R32G32B32A32_FLOAT",
    L"R16G16B16A16_FLOAT",
    L"R11G11B10_FLOAT" };
    EnumVar RTAO_TemporalCache_NormalDepthResourceFormat(L"Render/AO/RTAO/Temporal Cache/Texture Formats/NormalDepth resource", RGBResourceFormats::R11G11B10_FLOAT, RGBResourceFormats::Count, FloatingPointFormatsRGB, OnRecreateRaytracingResources);
...
void D3D12RaytracingAmbientOcclusion::CreateGBufferResources()
{
    ...
    // Full-res GBuffer resources.
    {
        ...
        CreateRenderTargetResource(device, RGBResourceFormats::ToDXGIFormat(SceneArgs::RTAO_TemporalCache_NormalDepthResourceFormat), m_GBufferWidth, m_GBufferHeight, m_cbvSrvUavHeap.get(), &m_GBufferResources[GBufferResource::NormalDepthLowPrecision], initialResourceState, L"GBuffer Normal Depth Low Precision");
        ...
        // ToDo use 16bit?
        CreateRenderTargetResource(device, RGBResourceFormats::ToDXGIFormat(SceneArgs::RTAO_TemporalCache_NormalDepthResourceFormat), m_GBufferWidth, m_GBufferHeight, m_cbvSrvUavHeap.get(), &m_GBufferResources[GBufferResource::ReprojectedHitPosition], initialResourceState, L"GBuffer Reprojected Hit Position");
    ...
    // Full-res Temporal Cache resources.
    {
        for (UINT i = 0; i < 2; i++)
        {
            ...
            CreateRenderTargetResource(device, RGBResourceFormats::ToDXGIFormat(SceneArgs::RTAO_TemporalCache_NormalDepthResourceFormat), m_GBufferWidth, m_GBufferHeight, m_cbvSrvUavHeap.get(), &m_temporalCache[i][TemporalCache::Normal], initialResourceState, L"Temporal Cache: Normal");
    ...
    CreateRenderTargetResource(device, texFormat, m_raytracingWidth, m_raytracingHeight, m_cbvSrvUavHeap.get(), &m_smoothedVarianceResource, initialResourceState, L"Smoothed Variance");
    m_meanResource.rwFlags = ResourceRWFlags::AllowWrite | ResourceRWFlags::AllowRead;
    CreateRenderTargetResource(device, texFormat, m_raytracingWidth, m_raytracingHeight, m_cbvSrvUavHeap.get(), &m_meanResource, initialResourceState, L"Mean");
    m_smoothedMeanResource.rwFlags = ResourceRWFlags::AllowWrite | ResourceRWFlags::AllowRead;
    CreateRenderTargetResource(device, texFormat, m_raytracingWidth, m_raytracingHeight, m_cbvSrvUavHeap.get(), &m_smoothedMeanResource, initialResourceState, L"Smoothed Mean");
...
void D3D12RaytracingAmbientOcclusion::OnUpdate()
{
    ...
    m_sceneCB->RTAO_UseSortedRays = SceneArgs::RTAOUseRaySorting;
...
void D3D12RaytracingAmbientOcclusion::ApplyMultiScaleAtrousWaveletTransformFilter()
{
    ...
    // Downsample input value.
    {
        ...
        RWGpuResource* inputResources[] = { &AOResources[AOResource::Coefficient], &GBufferResources[GBufferResource::NormalDepthLowPrecision] };
        ...
        for (int i = 0; i < SceneArgs::RTAODenoisingMultiscaleLevels; i++)
        {
            ...
            if (i == 0)
            {
                D3D12_RESOURCE_BARRIER preCopyBarriers[] = {
                    ...
                    CD3DX12_RESOURCE_BARRIER::Transition(GBufferResources[GBufferResource::NormalDepthLowPrecision].resource.Get(), D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, D3D12_RESOURCE_STATE_COPY_SOURCE),
                ...
                commandList->CopyResource(msResource.m_normalDepth.resource.Get(), GBufferResources[GBufferResource::NormalDepthLowPrecision].resource.Get());
                ...
                D3D12_RESOURCE_BARRIER postCopyBarriers[] = {
                    ...
                    CD3DX12_RESOURCE_BARRIER::Transition(GBufferResources[GBufferResource::NormalDepthLowPrecision].resource.Get(), D3D12_RESOURCE_STATE_COPY_SOURCE, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE),
...
void D3D12RaytracingAmbientOcclusion::RenderPass_GenerateGBuffers()
{
    ...
    commandList->SetComputeRootDescriptorTable(GlobalRootSignature::Slot::NormalDepthLowPrecision, m_GBufferResources[GBufferResource::NormalDepthLowPrecision].gpuDescriptorWriteAccess);
...
void D3D12RaytracingAmbientOcclusion::DownsampleGBuffer()
{
    ...
    // Transition all output resources to UAV state.
    {
        ...
        D3D12_RESOURCE_BARRIER barriers[] = {
            ...
            CD3DX12_RESOURCE_BARRIER::Transition(m_GBufferLowResResources[GBufferResource::NormalDepthLowPrecision].resource.Get(), before, after),
        };
        commandList->ResourceBarrier(ARRAYSIZE(barriers), barriers);
    }
    ...
    m_downsampleGBufferBilateralFilterKernel.Execute(
        ...
        m_GBufferHeight,
        m_cbvSrvUavHeap->GetHeap(),
        m_GBufferResources[GBufferResource::NormalDepthLowPrecision].gpuDescriptorReadAccess,
        ...
        m_GBufferLowResResources[GBufferResource::NormalDepthLowPrecision].gpuDescriptorWriteAccess,
...
void D3D12RaytracingAmbientOcclusion::UpsampleResourcesForRenderComposePass()
{
    ...
    if (inputLowResValueResource)
    {
        BilateralUpsample(
            ...
            m_GBufferLowResResources[GBufferResource::NormalDepthLowPrecision].gpuDescriptorReadAccess,
            m_GBufferResources[GBufferResource::NormalDepthLowPrecision].gpuDescriptorReadAccess,
            m_GBufferResources[GBufferResource::PartialDepthDerivatives].gpuDescriptorReadAccess,
...
void D3D12RaytracingAmbientOcclusion::RenderPass_BlurAmbientOcclusion()
{
	...
	commandList->SetComputeRootDescriptorTable(Slot::Normal, GBufferResources[GBufferResource::NormalDepthLowPrecision].gpuDescriptorReadAccess);
...
void D3D12RaytracingAmbientOcclusion::RenderPass_TemporalCacheReverseProjection()
{
   ...
   // XMMATRIX reverseProjectionTransform = invViewProj * cameraTranslation * prevView * prevProj;
    ...
    m_temporalCacheReverseReprojectKernel.Execute(
        ...
        static_cast<RGBResourceFormats::Type>(static_cast<UINT>(SceneArgs::RTAO_TemporalCache_NormalDepthResourceFormat)),
        m_debugOutput,
        m_camera.Eye(),
        invViewProj,
    ...
    if (SceneArgs::RTAO_UseTemporalCache)
    {
    ...
    ID3D12Resource* FrameNormalToCacheResourceHandle = SceneArgs::RTAO_TemporalCache_UseLowFidelityNormalDepthResource ?
        ...
        : GBufferResources[GBufferResource::NormalDepthLowPrecision].resource.Get();
    CopyTextureRegion(
        commandList,

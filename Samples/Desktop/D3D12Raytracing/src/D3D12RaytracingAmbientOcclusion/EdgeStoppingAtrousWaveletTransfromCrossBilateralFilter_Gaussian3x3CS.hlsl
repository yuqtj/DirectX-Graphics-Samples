...
void AddFilterContribution(inout float weightedValueSum, inout float weightSum, in float value, in float depth, in float3 normal, in uint row, in uint col, in float w_h, in uint2 DTid)
{
...
void main(uint2 Gid : SV_GroupID, uint2 GTid : SV_GroupThreadID, uint2 DTid : SV_DispatchThreadID)
{
    ...
    int2 pixel = PrefetchData(Gid, GTid);
    if (pixel.x >= cb.textureDim.x && pixel.y >= cb.textureDim.y)
    {
        return;
    }
    ...
    AddFilterContribution(weightedValueSum, weightSum, value, depth, normal, obliqueness, kernel[0][0], pixel, CTid, int2(-1, 1));
    AddFilterContribution(weightedValueSum, weightSum, value, depth, normal, obliqueness, kernel[1][1], pixel, CTid, int2( 0, 1));

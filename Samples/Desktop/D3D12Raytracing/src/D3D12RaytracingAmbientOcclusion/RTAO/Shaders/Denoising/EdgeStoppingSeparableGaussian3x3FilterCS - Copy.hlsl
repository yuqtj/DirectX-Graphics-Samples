...
groupshared uint PackedRowResultCache[16][8];            // 16bit float filteredValue, filteredVariance

groupshared uint PackedInputValueStdDeviationCache[16][8]; 
groupshared uint PackedInputDepthNormalXCache[16][8];
groupshared uint PackedInputNormalYZCache[16][8];
groupshared uint PackedInputDdxyCache[16][8];
...
void FilterHorizontally(in uint2 Gid, in uint GI)
{
    ...
    for (uint i = 0; i < NumRowsToLoadPerThread; i++)
    {
        ...
        if (GTid16x4.x < NumValuesToLoadPerRowOrColumn)
        {
            ...
            variance = g_inSmoothedVariance[pixel];
...
void FilterVertically(uint2 DTid, in uint2 GTid)
{
    ...
    // Load the values for the kernel's center.
    uint smIndex = GTid + (0, 8);
    uint unpackedValueStdDeviation = HalfToFloat2(PackedInputValueStdDeviationCache[smIndex.y][smIndex.x]);
    uint unpackedDepthNormalX = HalfToFloat2(PackedInputDepthNormalXCache[smIndex.y][smIndex.x]);
    uint unpackedNormalYZ = HalfToFloat2(PackedInputNormalYZCache[smIndex.y][smIndex.x]);
    uint unpackedDdxy = HalfToFloat2(PackedInputDdxyCache[smIndex.y][smIndex.x]);
    
    float value = unpackedValueStdDeviation.x;
    float stdDeviation = unpackedValueStdDeviation.y;
    float depth = unpackedDepthNormalX.x;
    float3 normal = float3(unpackedDepthNormalX.y, unpackedNormalYZ);
    float2 ddxy = unpackedDdxy;


    float value = WaveReadLaneAt(value, laneToReadFrom);
    float iDepth = WaveReadLaneAt(depth, laneToReadFrom);
    float3 iNormal = WaveReadLaneAt(normal, laneToReadFrom);
    float iVariance = WaveReadLaneAt(variance, laneToReadFrom);
    uint2 pixelOffset = uint2(i + 1, 0);
    ...
    for (int i = -FilterKernel::Radius; i < FilterKernel::Radius; i++)
    {
        if (i != 0)
        {
            float2 unpackedRowSum = HalfToFloat2(PackedRowResultCache[rowID][GTid.x]);
    #if OUTPUT_FILTERED_VARIANCE
            float2 unpackedRowSum2 = HalfToFloat2(PackedRowResultCache2[rowID][GTid.x]);
    #endif

            uint2 iSmIndex = smIndex + int2(0, i);
            uint laneToReadFrom = Row_BaseWaveLaneIndex + 1 + i +
                (GTid16x4.x < GroupDim.x ? GTid16x4.x : (GTid16x4.x - GroupDim.x) + FilterKernel::Radius);
            float iValue = WaveReadLaneAt(value, laneToReadFrom);
            float iDepth = WaveReadLaneAt(depth, laneToReadFrom);
        ...
        float w = FilterKernel::Kernel1D[i];

        weightSum += w * unpackedRowSum.x;
        weightedValueSum += unpackedRowSum.y;
#if OUTPUT_FILTERED_VARIANCE
        weightedVarianceSum = unpackedRowSum2.x;
#endif
    }

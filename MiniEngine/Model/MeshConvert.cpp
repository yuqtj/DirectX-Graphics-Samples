...
void OptimizeMesh( Renderer::Primitive& outPrim, const glTF::Primitive& inPrim, const Math::Matrix4& localToObject )
{
    ...
    ASSERT_SUCCEEDED(vbr.Read(position.get(), "POSITION", 0, vertexCount));
    {
        ...
        outPrim.m_BoundsLS = BoundingSphere(sphereCenterLS, Sqrt(maxRadiusLSSq));
        outPrim.m_BoundsOS = BoundingSphere(sphereCenterOS, Sqrt(maxRadiusOSSq));
    ...
    {
        if (b32BitIndices)
            ComputeNormals((uint32_t*)indices, indexCount * 3, position.get(), vertexCount, 0, normal.get());
        else
            ComputeNormals((uint16_t*)indices, indexCount * 3, position.get(), vertexCount, 0, normal.get());
    }

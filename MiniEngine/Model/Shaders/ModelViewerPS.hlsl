...
mrt.ID = HashTriangleID(vsOutput.vertexID);
	...
	uint2 pixelPos = uint2(vsOutput.position.xy * float2(2, 1));
	float2 ddxUV = 0.5 * ddx(vsOutput.uv);
	float2 ddyUV = ddy(vsOutput.uv) + (FrameIndexMod2 == 0 ? -ddxUV : +ddxUV);
# define SAMPLE_TEX(texName) texName.SampleGrad(defaultSampler, vsOutput.uv, ddxUV, ddyUV)
#else

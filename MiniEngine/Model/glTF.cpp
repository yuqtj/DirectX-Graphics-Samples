...
void glTF::Asset::ProcessMeshes( json& meshes, json& accessors )
{
    ...
    uint32_t curMesh = 0;
    for (json::iterator it = meshes.begin(); it != meshes.end(); ++it, ++curMesh)
    {
        json& thisMesh = it.value();
        ...
        for (json::iterator it = primitives.begin(); it != primitives.end(); ++it, ++curSubMesh)
        {
            glTF::Primitive& prim = m_meshes[curMesh].primitives[curSubMesh];
            json& thisPrim = it.value();
...
void glTF::Asset::ProcessImages( json& images )
{
    ...
    for (json::iterator it = images.begin(); it != images.end(); ++it)
    {
        ...
        {
            m_images[imageIdx++].path = thisImage.at("uri");
        }
